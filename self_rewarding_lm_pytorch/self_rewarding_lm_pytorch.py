...
from self_rewarding_lm_pytorch.cpo import CPO, CPOTrainer
...
class RewardGenerator(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.preference_seq_memmap = open_memmap(
            preference_seq_memmap_file, dtype="int", mode="w+", shape=memmap_shape
        )
        self.prompt_len_memmap = open_memmap(
            prompt_len_memmap, dtype="int", mode="w+", shape=(num_preference_pairs,)
        )

    def generate_reward(self, prompt: str, response: str) -> float:
        """
        ...
        reward_responses = wrapped_model.generate(
            ...
            filter_kwargs=dict(thres=self.eval_nucleus_p),
        )

        reward_responses_as_str: List[str] = [
            self.tokenizer_decode(resp[resp != self.pad_id].cpu())
            for resp in reward_responses
        ]
        rewards: List[Optional[float]] = [
            parse_reward(resp_str) for resp_str in reward_responses_as_str
        ]

        rewards = [
            *filter(exists, rewards)
        ]  # for now, just filter out any failed responses
    ...
    def forward(self) -> DPODataset:
        ...
        dpo_dataset = DPODataset(
            self.preference_seq_memmap_file, self.prompt_len_memmap_file
        )
        return dpo_dataset
...
class SelfRewardingTrainer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert all(
            [key in reward_prompt_config for key in reward_iteration_type]
        ), f"reward prompt must be one of {reward_prompt_config.keys()}"

        if not exists(model_generate_prompt):
            model_generate_prompt = deepcopy(model)

        self.reward_prompt_configs = [
            reward_prompt_config[key] for key in reward_iteration_type
        ]
        ...
        self.reward_generators = [
            RewardGenerator(
                model=model,
                model_generate_prompt=model_generate_prompt,
                reward_config=reward_config,
                num_preference_pairs=one_stage_num_preference_pairs,
                **reward_generator_kwargs,
            )
            for reward_config, one_stage_num_preference_pairs in zip(
                self.reward_prompt_configs, num_preference_pairs
            )
        ]
        ...
        if use_cpo:
            ...
            self.trainer = CPOTrainer(
                cpo=self.dpo, accelerator=self.accelerator, **dpo_trainer_kwargs
            )
        else:
            self.dpo = DPO(model)

            self.trainer = DPOTrainer(
                dpo=self.dpo, accelerator=self.accelerator, **dpo_trainer_kwargs
            )
    ...
    def forward(self):
        ...
        for ind, reward_generator in enumerate(range(self.reward_generators)):
            ...
            self.trainer(self_reward_dataset)
